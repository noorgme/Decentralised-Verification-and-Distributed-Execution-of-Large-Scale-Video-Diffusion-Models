...
# Plot sanity checks for each gamma
for gi, g in enumerate(gammas):
    # Plot value consistency
    title = f"Value Consistency Check γ={g}"
    consistency = np.abs(EVc[gi] - EVh[gi] - Delta[gi])
    plot_heatmap(consistency.T, title, f"value_consistency_gamma{g}.png", cmap="YlOrRd")
    
    # Plot value bounds
    title = f"Value Bounds Check γ={g}"
    bounds_check = np.logical_and(EVc[gi] >= 0, EVh[gi] >= 0)
    plot_heatmap(bounds_check.T, title, f"value_bounds_gamma{g}.png", cmap="binary") 